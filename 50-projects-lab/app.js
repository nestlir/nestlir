...
function createCard(project) {
  if (project.kind === 'system') return `<article class="card system-card"><div class="system-mark">C/C++</div><div class="card-body"><small>SYSTEMS LAB</small><h2>${project.name}</h2><p>${project.description}</p><div><button class="open" data-system="${project.slug}">Explore →</button><a href="https://github.com/nestlir/nestlir/tree/main/50-projects-lab/projects/c-cpp/${project.slug}" target="_blank" rel="noreferrer">Source →</a></div></div></article>`;
...
function openSystem(slug) {
  ...
  const source = `https://github.com/nestlir/nestlir/tree/main/50-projects-lab/projects/c-cpp/${slug}`;
  app.innerHTML = `<section class="demo wrap"><header><button id="back">← Lab</button><span>C/C++ SYSTEMS LAB</span><a target="_blank" rel="noreferrer" href="${source}">Native source ↗</a></header><div class="demo-head"><small>INTERACTIVE ARCHITECTURE EXPLAINER</small><h1>${project.name}</h1><p>${project.description}. The browser demo explains the native execution model; it does not replace the C/C++ binary.</p></div><div class="stage"><div><div id="diagram" class="diagram" aria-live="polite"></div><button id="step">Advance one step</button><div id="state"></div></div></div></section>`;

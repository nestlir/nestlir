...
function rateLimited(req) { const key = clientIp(req); const now = Date.now(); const current = hits.get(key); if (!current || now - current.start >= 60_000) { hits.set(key, { start: now, count: 1 }); return false; } current.count += 1; return current.count > 30; }
async function body(req, max = 16_000) { return new Promise((resolve, reject) => { let size = 0; const chunks = []; req.on('data', (chunk) => { size += chunk.length; if (size > max) { reject(Object.assign(new Error('Payload too large'), { statusCode: 413 })); req.destroy(); return; } chunks.push(chunk); }); req.on('end', () => resolve(Buffer.concat(chunks).toString('utf8'))); req.on('error', reject); }); }
...
async function persist(orders) { await fs.mkdir(DATA_DIR, { recursive: true }); const tmp = `${ORDERS_FILE}.tmp`; await fs.writeFile(tmp, JSON.stringify(orders.slice(-500), null, 2), 'utf8'); await fs.rename(tmp, ORDERS_FILE); }
...
export function createServer() { return http.createServer(async (req, res) => {
  const requestId = randomUUID(); res.setHeader('x-request-id', requestId); res.setHeader('access-control-allow-origin', CORS_ORIGIN); res.setHeader('access-control-allow-methods', 'GET,POST,OPTIONS'); res.setHeader('access-control-allow-headers', 'content-type');
  if (req.method === 'OPTIONS') { res.writeHead(204); res.end(); return; }
  if (rateLimited(req)) return json(res, 429, { ok: false, error: { code: 'RATE_LIMITED', message: 'Too many requests.', requestId } });
  ...
  try {
    ...
    if (req.method === 'POST' && url.pathname === '/api/orders') { let input; try { input = JSON.parse(await body(req) || '{}'); } catch { return json(res, 400, { ok: false, error: { code: 'INVALID_JSON', message: 'Request body must be valid JSON.', requestId } }); } const result = validateOrder(input); if (!result.ok) return json(res, 400, { ok: false, error: { code: 'VALIDATION_ERROR', message: result.error, requestId } }); const order = await createOrder(result.orderInput); return json(res, 201, { ok: true, order, requestId }); }
...
if (entrypoint === currentFile) createServer().listen(PORT, HOST, () => console.log(`MOMOHA API listening on http://${HOST}:${PORT}`));
